...
class FoodDrinksController {
  ...
  async add(req, res) {
    ...
    const duplicateName = await FoodsAndDrinks.findOne({
    ...
    if (duplicateName)
      ...
      if (allComboBySlug.length > 0) {
        let filterLastCombo =
          allComboBySlug[allComboBySlug.length - 1].tenCombo.split("-");
        let slugNumber = Number(filterLastCombo[filterLastCombo.length - 1]);
        if (slugNumber) slugNumber++;
        else slugNumber = 1;
        slug += `-${slugNumber}`;
      }
  ...
  async update(req, res) {
    const comboInfo = await FoodsAndDrinks.findOne({
    ...
    if (comboInfo) {
      ...
      FoodsAndDrinks.findOneAndUpdate(
        { biDanh: req.params.bidanh },
        comboUpdate
      )
        .then((data) => {
        ...
        .catch((err) => {
          return res
            .status(500)
            .json({ error: "Hệ thống đang xử lý, vui lòng chờ" });
        });
    }
  }

  async getDetail(req, res) {
    let detail = await FoodsAndDrinks.findOne({
      biDanh: req.params.bidanh,
      daXoa: false,
    });
